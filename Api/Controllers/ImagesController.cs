...
namespace Api.Server.Controllers
{
    ...
    public class ImagesController : ControllerBase
    {
        ...
        public async Task<ActionResult> GetById(int id)
        {
            ...
            if (image == null)
                return NotFound($"תמונה עם מזהה {id} לא נמצאה");
            return Ok(_mapper.Map<ImagePostModel>(image));
        }

        // הוספת תמונה (ללא קובץ, רק נתונים)
        [HttpPost]
        public async Task<ActionResult> Add([FromBody] ImagePostModel image)
        {
            if (image == null)
            ...
            var photo = _mapper.Map<Images>(image);
            await _imageService.AddValueAsync(photo);

            return CreatedAtAction(nameof(GetById), new { id = photo.Id }, _mapper.Map<ImagePostModel>(photo));
        }

        // עדכון
        [HttpPut("{id}")]
        public async Task<ActionResult> Update(int id, [FromBody] UpdateImagesModel model)
        {
            if (model == null)
                return BadRequest("נתונים שגויים");
            ...
            image.FileUrl = model.FileUrl;
            image.FileType = model.FileType;
            await _imageService.PutValueAsync(image);

            return Ok(_mapper.Map<ImagePostModel>(image));
        }

        // מחיקה ממסד נתונים בלבד
        [HttpDelete("{id}")]
        public async Task<ActionResult> Delete(int id)
        {
            var image = await _imageService.GetByIdAsync(id);
        ...
        // העלאת קובץ ל־S3
        [HttpPost("upload-to-s3")]
        public async Task<IActionResult> UploadToS3([FromForm] ImagePostModel model)
        {
            ...
            try
            {
                ...
                if (model.File.Length > maxSize)
                    return BadRequest($"גודל הקובץ חורג מהמגבלה של {maxSize / (1024 * 1024)}MB");

                var fileUrl = await _s3Service.UploadFileAsync(model.File, "images/");
        ...
        public async Task<IActionResult> GetS3FileInfo([FromQuery] string fileUrl)
        {
            ...
            {
                return StatusCode(500, $"שגיאה בקבלת מטא-דאטה: {ex.Message}");
            }
        }

        // מחיקה מ־S3 וממסד הנתונים
        [HttpDelete("delete-from-s3/{id}")]
        public async Task<IActionResult> DeleteFromS3AndDb(int id)
        {
            try
            {
                ...
                {
                    message = exists
                        ? "הקובץ והתמונה נמחקו בהצלחה"
                        : "התמונה נמחקה ממסד הנתונים, אך הקובץ לא נמצא ב-S3"
                });
            }
            catch (Exception ex)
            {
